...
```{r}
data_new <- clean_df |>
  filter(!is.na(work == 1), !is.na(party_text), !is.nan(work == 1), !is.nan(party_text), is.finite(work == 1), is.finite(party_text))
data_new



work_fit <- lm(work_text ~ party_text, data = data_new) 
work_fit 

```

```{r}
modelsummary::modelsummary(lm1, statistic = c("s.e. = {std.error}", "p = {p.value}"), gof_map = c("nobs", "r.squared", "adj.r.squared"))
```
